<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h2 class="font-bold text-2xl text-secondary-900 leading-tight">Responses</h2>
                <p class="text-sm text-secondary-600 mt-1">Form: <span
                        class="font-semibold text-primary-600">{{ $form->title }}</span></p>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.forms.index') }}"
                    class="inline-flex items-center gap-2 rounded-lg border border-secondary-300 bg-white px-4 py-2 text-sm font-medium text-secondary-700 hover:bg-secondary-50 transition-all">
                    <i class="bi bi-arrow-left"></i>
                    <span>Daftar Form</span>
                </a>
                <a href="{{ route('admin.forms.responses.export', $form) }}"
                    class="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-success-600 to-success-500 px-4 py-2 text-sm font-semibold text-white shadow-soft hover:shadow-soft-md transition-all">
                    <i class="bi bi-download"></i>
                    <span>Export CSV</span>
                </a>
            </div>
        </div>
    </x-slot>

    <style>
        [x-cloak] {
            display: none !important
        }
    </style>

    @php
        // Siapkan data ringan untuk UI client-side (halaman ini saja)
        $rows = $responses
            ->map(function ($resp) {
                $email = $resp->respondent_email ?? (optional($resp->respondent)->email ?? '-');
                $name = optional($resp->respondent)->name ?? '-';
                $dur = $resp->duration_seconds ? gmdate('H:i:s', $resp->duration_seconds) : '-';
                return [
                    'id' => $resp->id,
                    'seq' => null, // dihitung di client
                    'uid' => $resp->uid,
                    'name' => $name,
                    'email' => $email,
                    'status' => $resp->status,
                    'submitted_at' => optional($resp->submitted_at)?->format('Y-m-d H:i:s'),
                    'duration' => $dur,
                    'show_url' => route('admin.responses.show', $resp),
                ];
            })
            ->values();
        $firstSeq = $responses->firstItem() ?? 1;
    @endphp

    <div class="py-8" x-data="respIndex({
        rows: @js($rows),
        firstSeq: {{ (int) $firstSeq }},
    })" x-init="init()">
        <div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">

            @if (session('status'))
                <div class="rounded-xl border border-emerald-200 bg-emerald-50/60 p-3 text-emerald-900">
                    {{ session('status') }}
                </div>
            @endif

            <div class="rounded-2xl border border-emerald-100 bg-white shadow-sm">
                <!-- Toolbar -->
                <div class="flex flex-col gap-3 p-6 md:flex-row md:items-end md:justify-between">
                    <div class="grid w-full grid-cols-1 gap-3 md:grid-cols-3">
                        <div>
                            <label class="block text-xs font-medium text-emerald-900">Cari</label>
                            <input x-model.debounce.200ms="q" type="text" placeholder="Cari nama, email, UID…"
                                class="mt-1 w-full rounded-xl border border-emerald-200 px-3 py-2 placeholder:text-emerald-900/40 focus-visible:ring-2 focus-visible:ring-emerald-300">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-emerald-900">Status</label>
                            <select x-model="status"
                                class="mt-1 w-full rounded-xl border border-emerald-200 bg-white px-3 py-2 focus-visible:ring-2 focus-visible:ring-emerald-300">
                                <option value="">Semua</option>
                                <template x-for="s in statuses" :key="s">
                                    <option :value="s" x-text="s"></option>
                                </template>
                            </select>
                        </div>
                        <div class="flex gap-2">
                            <div class="flex-1">
                                <label class="block text-xs font-medium text-emerald-900">Dari (Submit)</label>
                                <input x-model="dFrom" type="date"
                                    class="mt-1 w-full rounded-xl border border-emerald-200 px-3 py-2 focus-visible:ring-2 focus-visible:ring-emerald-300">
                            </div>
                            <div class="flex-1">
                                <label class="block text-xs font-medium text-emerald-900">Sampai</label>
                                <input x-model="dTo" type="date"
                                    class="mt-1 w-full rounded-xl border border-emerald-200 px-3 py-2 focus-visible:ring-2 focus-visible:ring-emerald-300">
                            </div>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        <button @click="resetFilters"
                            class="inline-flex items-center rounded-xl border border-emerald-200 bg-white px-3 py-2 text-sm text-emerald-800 hover:bg-emerald-50">
                            Reset
                        </button>
                        <div class="text-xs text-emerald-700/70" x-text="`${filtered.length} ditampilkan`"></div>
                    </div>
                </div>

                <!-- Tabel -->
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm">
                        <thead class="text-left text-emerald-700/80">
                            <tr class="border-y border-emerald-100">
                                <th class="sticky left-0 z-10 bg-white py-2 pr-4 pl-6">#</th>
                                <th class="py-2 pr-4">UID</th>
                                <th class="py-2 pr-4">User</th>
                                <th class="hidden py-2 pr-4 md:table-cell">Email</th>
                                <th class="py-2 pr-4">Status</th>
                                <th class="hidden py-2 pr-4 lg:table-cell">Submitted At</th>
                                <th class="hidden py-2 pr-4 md:table-cell">Duration</th>
                                <th class="py-2 pr-6">Tools</th>
                            </tr>
                        </thead>
                        <tbody>
                            <template x-if="filtered.length === 0">
                                <tr>
                                    <td colspan="8" class="py-6 text-center text-emerald-700/70">
                                        Tidak ada data yang cocok dengan filter.
                                    </td>
                                </tr>
                            </template>

                            <template x-for="row in filtered" :key="row.id">
                                <tr class="border-t border-emerald-100 hover:bg-emerald-50/40">
                                    <!-- seq -->
                                    <td class="sticky left-0 z-10 bg-white py-2 pr-4 pl-6" x-text="row.seq"></td>

                                    <!-- uid + copy -->
                                    <td class="py-2 pr-4">
                                        <div class="flex items-center gap-2">
                                            <code class="text-xs text-emerald-900/80" x-text="row.uid"></code>
                                            <button @click="copy(row.uid)"
                                                class="rounded-lg border border-emerald-200 bg-white px-2 py-0.5 text-[11px] text-emerald-800 hover:bg-emerald-50">
                                                Copy
                                            </button>
                                        </div>
                                    </td>

                                    <!-- name -->
                                    <td class="py-2 pr-4" x-text="row.name"></td>

                                    <!-- email -->
                                    <td class="hidden py-2 pr-4 md:table-cell" x-text="row.email"></td>

                                    <!-- status chip -->
                                    <td class="py-2 pr-4">
                                        <span class="inline-flex items-center rounded-full px-2.5 py-1 text-xs"
                                            :class="row.status === 'submitted' ?
                                                'bg-emerald-100 text-emerald-800' :
                                                'bg-yellow-100 text-yellow-800'">
                                            <span x-text="row.status"></span>
                                        </span>
                                    </td>

                                    <!-- submitted -->
                                    <td class="hidden py-2 pr-4 lg:table-cell" x-text="row.submitted_at || '-'"></td>

                                    <!-- duration -->
                                    <td class="hidden py-2 pr-4 md:table-cell" x-text="row.duration"></td>

                                    <!-- tools -->
                                    <td class="py-2 pr-6">
                                        <div class="flex items-center gap-2">
                                            <a :href="row.show_url"
                                                class="inline-flex items-center rounded-xl border border-emerald-200 bg-white px-3 py-1.5 hover:bg-emerald-50">
                                                Detail
                                            </a>
                                            <!-- tambahkan tools lain jika perlu -->
                                        </div>
                                    </td>
                                </tr>
                            </template>
                        </tbody>
                    </table>
                </div>

                <!-- Footer: pagination laravel tetap -->
                <div class="p-4">
                    {{ $responses->links() }}
                </div>
            </div>

            <div class="rounded-2xl border border-emerald-100 bg-white shadow-sm p-6">
                <h3 class="mb-2 font-semibold text-emerald-900">Tips</h3>
                <ul class="list-inside list-disc space-y-1 text-sm text-emerald-800">
                    <li>Gunakan <strong>Export CSV</strong> untuk mengambil semua respons.</li>
                    <li>Durasi dihitung dari <em>started_at</em> ke <em>submitted_at</em> (jika tersedia).</li>
                    <li>Filter di atas bekerja untuk data pada halaman saat ini.</li>
                </ul>
            </div>

        </div>
    </div>

    <script>
        function respIndex(cfg) {
            return {
                // raw rows dari server (halaman ini)
                rows: cfg.rows || [],
                firstSeq: cfg.firstSeq || 1,

                // filters
                q: '',
                status: '',
                dFrom: '',
                dTo: '',

                statuses: [],

                init() {
                    // set nomor urut default (sebelum filter)
                    this.rows.forEach((r, i) => r.seq = this.firstSeq + i);
                    // daftar status unik
                    const set = new Set(this.rows.map(r => r.status).filter(Boolean));
                    this.statuses = Array.from(set);
                },

                get filtered() {
                    let arr = this.rows.slice();

                    // cari (uid/name/email)
                    const q = (this.q || '').toLowerCase().trim();
                    if (q) {
                        arr = arr.filter(r =>
                            String(r.uid).toLowerCase().includes(q) ||
                            String(r.name).toLowerCase().includes(q) ||
                            String(r.email).toLowerCase().includes(q)
                        );
                    }

                    // status
                    if (this.status) {
                        arr = arr.filter(r => r.status === this.status);
                    }

                    // date range (submitted_at)
                    if (this.dFrom) {
                        const from = new Date(this.dFrom + 'T00:00:00');
                        arr = arr.filter(r => r.submitted_at ? (new Date(r.submitted_at) >= from) : false);
                    }
                    if (this.dTo) {
                        const to = new Date(this.dTo + 'T23:59:59');
                        arr = arr.filter(r => r.submitted_at ? (new Date(r.submitted_at) <= to) : false);
                    }

                    // urutkan submitted_at desc (null di bawah)
                    arr.sort((a, b) => {
                        const da = a.submitted_at ? new Date(a.submitted_at).getTime() : 0;
                        const db = b.submitted_at ? new Date(b.submitted_at).getTime() : 0;
                        return db - da;
                    });

                    // re-sequence sesuai hasil filter
                    arr.forEach((r, i) => r.seq = this.firstSeq + i);
                    return arr;
                },

                resetFilters() {
                    this.q = '';
                    this.status = '';
                    this.dFrom = '';
                    this.dTo = '';
                },

                async copy(text) {
                    try {
                        await navigator.clipboard.writeText(String(text || ''));
                    } catch (e) {}
                }
            }
        }
    </script>
</x-app-layout>

<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h2 class="font-bold text-2xl text-secondary-900 leading-tight">Pertanyaan</h2>
        ...
        <div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
            @forelse($form->sections as $section)
                <div class="bg-white rounded-xl shadow-soft border border-primary-100 overflow-hidden">
                    <!-- Section Header -->
                    <div class="bg-gradient-to-r from-primary-50 to-primary-100/50 px-6 py-4 border-b border-primary-200">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <span
                                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-primary-600 text-white text-sm font-semibold">
                                    {{ $section->position }}
                                </span>
                                <div>
                                    ...
                                    @if ($section->description)
                                        <p class="text-sm text-secondary-600 mt-0.5">{{ $section->description }}</p>
                                    @endif
                                </div>
                            </div>
                    ...
                    <div class="p-6">
                        @if ($section->questions->isEmpty())
                            <div class="text-center py-8">
                                <i class="bi bi-question-circle text-4xl text-secondary-300"></i>
                                <p class="text-sm text-secondary-500 mt-2">Belum ada pertanyaan di section ini</p>
                            </div>
                        @else
                            {{-- DESKTOP TABLE --}}
                                ...
                                <div class="overflow-x-auto rounded-lg border border-primary-100">
                                    <table class="min-w-full divide-y divide-primary-100">
                                        <thead class="bg-secondary-50">
                                            <tr>
                                                <th scope="col"
                                                    class="py-3 pl-6 pr-3 text-left text-xs font-semibold text-secondary-900 uppercase tracking-wider">
                                                    Pos</th>
                                                <th scope="col"
                                                    class="px-3 py-3 text-left text-xs font-semibold text-secondary-900 uppercase tracking-wider">
                                                    Tipe</th>
                                                <th scope="col"
                                                    class="px-3 py-3 text-left text-xs font-semibold text-secondary-900 uppercase tracking-wider">
                                                    Pertanyaan</th>
                                                <th scope="col"
                                                    class="py-3 pl-3 pr-6 text-right text-xs font-semibold text-secondary-900 uppercase tracking-wider">
                                                    Aksi</th>
                                            </tr>
                                        </thead>
                                            ...
                                            @foreach ($section->questions as $q)
                                                <tr class="hover:bg-primary-50/50 transition-colors">
                                                    <td class="py-3 pl-6 pr-3">
                                                        <span
                                                            class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-primary-100 text-primary-700 text-sm font-semibold">
                                                            {{ $q->position }}
                                                        </span>
                                                    </td>
                                                    <td class="px-3 py-3">
                                                        <span
                                                            class="inline-flex items-center rounded-full bg-secondary-100 px-2.5 py-1 text-xs font-medium text-secondary-700">
                                                            {{ strtoupper(str_replace('_', ' ', $q->type)) }}
                                                        </span>
                                                    </td>
                                                    <td class="px-3 py-3">
                                                        <a href="{{ route('admin.questions.show', $q) }}"
                                                            class="text-secondary-900 hover:text-primary-600 font-medium transition-colors line-clamp-2">
                                                            {{ $q->title }}
                                                        </a>
                                                        @if ($q->is_required)
                                                            <span
                                                                class="inline-flex items-center gap-1 text-xs text-danger-600 mt-1">
                                                                <i class="bi bi-asterisk"></i>
                                                                Required
                                                            </span>
                                                        @endif
                                                    </td>
                                                    <td class="py-3 pl-3 pr-6">
                                                            ...
                                                            <div class="relative" x-data="{ open: false }"
                                                                ...
                                                                <div x-show="open"
                                                                    ...
                                                                    <div class="p-1.5 space-y-0.5">
                                                                        ...
                                                                        <div class="my-1 border-t border-primary-100">
                                                                        </div>
                            ...
                            <div class="md:hidden space-y-3">
                                @foreach ($section->questions as $q)
                                    <div
                                        class="border border-primary-200 rounded-lg p-4 hover:bg-primary-50/50 transition-colors">
                                        ...
                                        <a href="{{ route('admin.questions.show', $q) }}"
                                            class="text-sm font-medium text-secondary-900 hover:text-primary-600 transition-colors block mb-2">
                                            {{ $q->title }}
                                        </a>

                                        @if ($q->is_required)
                                            <span class="inline-flex items-center gap-1 text-xs text-danger-600 mb-3">
                                                <i class="bi bi-asterisk"></i>
                                                Required
                                            </span>
                                        @endif
            ...
            @empty
                ...
                <div class="bg-white rounded-xl shadow-soft border border-primary-100 p-12 text-center">
                    ...
                    <p class="text-sm text-secondary-600 mb-6">Buat section terlebih dahulu sebelum menambahkan
                        pertanyaan</p>

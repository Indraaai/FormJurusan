<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl leading-tight text-emerald-900">
                Tambah Validation — Q#{{ $question->position }} ({{ $question->title }})
            </h2>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-3xl space-y-6 sm:px-6 lg:px-8">
            ...
            @php
                $oldType = old('validation_type', 'text_length');
                $oldEx = old('extras', isset($extras) && is_array($extras) ? $extras : []);
                $oldMimes = is_array($oldEx['mimes'] ?? null) ? implode(',', $oldEx['mimes']) : $oldEx['mimes'] ?? '';
                $oldMts = is_array($oldEx['mimetypes'] ?? null)
                    ? implode(',', $oldEx['mimetypes'])
                    : $oldEx['mimetypes'] ?? '';
            @endphp

            <div class="rounded-2xl border border-emerald-100 bg-white shadow-sm" x-data="valCreate({
                t: @js($oldType),
                // old/existing values
                minValue: @js(old('min_value')),
                maxValue: @js(old('max_value')),
                pattern: @js(old('pattern')),
                minDate: @js(old('extras.min_date', $oldEx['min_date'] ?? '')),
                maxDate: @js(old('extras.max_date', $oldEx['max_date'] ?? '')),
                minTime: @js(old('extras.min_time', $oldEx['min_time'] ?? '')),
                maxTime: @js(old('extras.max_time', $oldEx['max_time'] ?? '')),
                mimes: @js($oldMimes),
                mtypes: @js($oldMts),
                ...
                <form method="POST" action="{{ route('admin.questions.validations.store', $question) }}"
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        {{-- REQUIRED: tidak butuh field tambahan --}}
            ...
            <div class="rounded-2xl border border-emerald-100 bg-white shadow-sm p-6">
                ...
                <ul class="list-inside list-disc space-y-1 text-sm text-emerald-800">
                    <li><strong>text_length/number_range</strong>: isi min/max sesuai kebutuhan.</li>
                    <li><strong>file_size</strong>: gunakan <em>max_value</em> (KB). <em>min_value</em> akan diabaikan.
                    </li>
                    <li><strong>regex</strong>: pakai pola valid; hindari delimiter yang bikin bentrok.</li>
                    <li><strong>date_range/time_range</strong>: kosongkan salah satu jika ingin “hanya batas min” atau
                        “hanya batas max”.</li>
                    <li><strong>file_type</strong>: boleh isi <em>mimes</em> atau <em>mimetypes</em>, atau keduanya.
                    </li>
                    <li><strong>required</strong>: tidak perlu parameter tambahan.</li>
                </ul>
            </div>
    ...
    <script>
        function valCreate(cfg) {
            return {
                ...
                // Validasi ringan sebelum submit (opsional, backend tetap sumber kebenaran)
                validateClient() {
